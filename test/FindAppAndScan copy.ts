...
class FindAppAndScan {
    ...
    private static async writeIssuesToCSV(issues: CompatabilityIssue[], outputDir: string): Promise<void> {
        const outputPath = join(outputDir, 'ArkCiDOut.csv');
        if (!this.csvHeaderWritten) {
            ...
            await fs.writeFile(outputPath, header, { flag: 'w' });  // Overwrite file if existing
        ...
        if (rows.length > 0) {
            await fs.appendFile(outputPath, rows + '\n');  // Append issues to file
        }
    }
...
const input_dir = 'G:/cdh/CommitHistory/github';
const apiLifecycleDir = './resources';

FindAppAndScan.findAppsAndScan(input_dir, apiLifecycleDir)
